...
{
    private readonly FileSystemWatcher _modelWatcher;
    private readonly FileSystemWatcher[] _adapterWatchers;
    private readonly Timer _modelTimer;
        ...
        ComponentReloadCoordinator coordinator)
    {
        ...
        _modelTimer = new Timer(state => _ = ReloadModelsAsync(), null, Timeout.Infinite, Timeout.Infinite);
        _adapterTimer = new Timer(state => _ = ReloadAdapterAsync(), null, Timeout.Infinite, Timeout.Infinite);

        Directory.CreateDirectory(modelRoot);
        _modelWatcher = CreateWatcher(modelRoot, includeSubdirectories: true, ScheduleModelReload);
    ...
    private void ScheduleModelReload(string path)
    {
        if (path.EndsWith(".dll", StringComparison.OrdinalIgnoreCase))
        {
            _modelTimer.Change(750, Timeout.Infinite);
        }
    }
    ...
    private async Task ReloadModelsAsync()
    {
        ...
        try
        {
            await _coordinator.ReloadModelsAsync().ConfigureAwait(false);
            BotLog.Info("检测到 Model 文件变化，热重载完成。");
        }
        catch (Exception ex)
        {
            BotLog.Error("Model 自动热重载失败: " + ex.Message);
        }
    }
